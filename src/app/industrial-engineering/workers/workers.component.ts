...
import { AddEditWorkersComponent } from './add-edit-workers/add-edit-workers.component';
...
export class WorkersComponent {
  ...
  ngOnInit(): void {
    const perm$ = this.store.select(selectPermissionByMenu('IE Workers'));
  ...
  get(): void {
    ...
    this.http
      .get(`${environment.apiUrl}/api/WageWorker/GetWageWorkerList`)
      .subscribe((res: any) => {
        if (res.success) {
        ...
        this.toastr.error('Failed to load workers.', 'Error');
        this.loadingIndicator = false;
      });
  }
  ...
  onFilterField(value: string): void {
    ...
    this.gridView = this.gridData.filter((d: any) =>
      ...
      (d.skillLevel && d.skillLevel.toLowerCase().includes(q))
    );
  }
 
  add(): void {
    const modalRef = this.modalService.open(AddEditWorkersComponent, {
      centered: true,
  ...
  onDelete(row: any): void {
    ...
    this.http
      .delete(`${environment.apiUrl}/api/WageWorker/DeleteWageWorker/${row.id}`)
      .subscribe((res: any) => {
